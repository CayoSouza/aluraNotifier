...
## Edite este dicionario para customizar as secoes a serem monitoradas ##
...
def checkForDoubtsWithOutAnswer(seconds):
    ...
    while ( len(SECTIONS_TO_VERIFY) > 0 ):
        for secao, url in SECTIONS_TO_VERIFY.copy().items():
            ...
            print('>>Verificando %s<<' % secao)

            soup = BeautifulSoup(result, 'html.parser')

            no_threads_alert_message = soup.find('p', attrs={'class':'alert-message'})

            if not(no_threads_alert_message):
                ...
                ret_value = ctypes.windll.user32.MessageBoxW(0, "Existe(m) " + str(len(forumListItems)) +" dúvida(s) "
                                                             ...
                                                             , "Alura Notifier", 3)
                print('%s dúvida(s) em %s' % (len(forumListItems), secao))
...
checkForDoubtsWithOutAnswer(60)
